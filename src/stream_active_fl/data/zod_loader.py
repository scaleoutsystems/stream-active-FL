"""
ZOD Frame-Level Dataset.

Provides a PyTorch Dataset that wraps ZodSequences and per-sequence annotation JSONs
produced by Detectron2 (via tools/labeling/annotate.py).

Supports:
  - Train/val splits via ZOD's official split.
  - Frame subsampling (use every Nth frame).
  - Binary classification target: does frame contain object of a given category?
  - Optional: per-detection confidence scores for teacher-confidence gating later.
"""
